...
The compiler is implemented entirely in Rust and leverages LLVM to generate efficient machine code targeting multiple platforms, including macOS, Linux, and Windows. Core components of the compiler include a custom-built lexer, parser, and code generator. The language currently supports a wide range of fundamental constructs (such as functions, structs, control flow, and operator overloading), and a handful of basic types (integer, float, string, and boolean). Memory management is handled automatically through reference counting, eliminating the need for a garbage collector or manual memory management.

Overall, the prototype demonstrates the feasibility and potential of Reticulated as a performant, cross-platform, and user-friendly programming language. The compiler can serve as the foundation for future development of a more comprehensive compiler.

## Introduction

As its medium, programming languages are the foundation of software development. Unsurprisingly, beginners in the field often use simple, more abstract languages. One of the most popular (if not the most popular) of these languages is Python. It is powerful, simple, and easy to learn. However, Python is not ideal for all use cases. In particular, Python is dynamically typed and interpreted, lowering performance and making some bugs difficult to catch.

This project focuses on developing a cross-platform compiler for a subset of the Reticulated language. This language aims to be similar to Python in syntax and semantics while also providing the benefits of a statically typed, compiled language. Although the compiler does not support all language features, it contains the fundamental language constructs (i.e. assignments, functions, structs, while loops, if statements, etc.) and can be used as a starting point for future work. This prototype compiler also has cross-platform support by leveraging LLVM in its backend. LLVM is a collection of compiler toolchain technologies with a number of capabilities [1], and it is used in this project to generate efficient machine code for multiple platforms and architectures.
...
Reticulated's syntax closely mirrors Python, using nearly identical keywords, operators, and constructs [2]. However, there are a few notable exceptions:
...
-   Type hints are mandatory. This is an optional feature in Python, but necessary for Reticulated as it is strongly typed.

Furthermore, operations in Reticulated are implemented as dunder method calls, similar to Python (e.g., `__add__`, `__sub__`). For example, the expression a + b is internally translated into a function call `a.__add__(b)`. During code generation, these operations are resolved by looking up the appropriate dunder method based on the left-hand operand’s type. If the method exists, the compiler emits a call to the corresponding LLVM function. This system allows not only built-in types but also user-defined types to customize behavior for arithmetic, comparison, and string operations. A simple Reticulated example program demonstrating these features can be found in [Appendix A](#appendix-a---example-program).
...
The lexing process is performant and almost entirely context-independent. It works by reading characters from the input one at a time until the pattern for a token is found. Once a pattern is matched, the lexer invokes a handler to consume the characters from the input and construct an instance of the corresponding token object. If an unknown pattern or invalid sequence is encountered, the lexer raises a syntax error. The lexer only passes over the input once, meaning its time complexity is linear relative to the length of the source code.

### Parser

The parser is the second component of the compiler. It takes in tokens from the lexer and converts them into an AST. The AST is a tree representation of the source code, where each node represents a construct in the language. Figure 3 shows an example AST produced by the parser.
...
For each type, a reference count field is automatically added to the end of its struct. Additionally, user-inaccessible `copyptr` and `freeptr` functions are created for each type on compilation. These functions are then called anytime a reference is created or destroyed respectively.
...
<pre>
[1]  LLVM Foundation, “The LLVM Compiler Infrastructure Project,” Llvm.org, 2019. https://llvm.org/

[2]  python, “The Python Tutorial — Python 3.7.4 documentation,” Python.org, 2019. https://docs.python.org/3/tutorial/index.html

[3]  TheDan64, “GitHub - TheDan64/inkwell: It’s a New Kind of Wrapper for Exposing LLVM (Safely),” GitHub, Aug. 04, 2024. https://github.com/TheDan64/inkwell (accessed Apr. 11, 2025).

[4]  LLVM Foundation, “LLVM Language Reference Manual — LLVM 16.0.0git documentation,” llvm.org. https://llvm.org/docs/LangRef.html
</pre>
